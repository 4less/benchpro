...
monophyly <- function(tree, tree.group) {
  ...
  for (group in groups) {
    ...
    # print(paste("Group Tips", group.tips))
    lca <- getMRCA(tree, group.tips[group.tips %in% tree$tip.label])
    # print(paste("Extract", lca))
...
map.file <- "/usr/users/QIB_fr017/fritsche/Projects/benchpro/data/SPECIES46/map.csv"
...
if (FALSE) {
  for (treepath in map[map$Tool == "gold_std", "SpeciesTree"]) {
    print(treepath)
  }
for (treepath in map[map$Tool == "gold_std", "SpeciesTree"]) {
  if (!file.exists(treepath)) next
  print(treepath)
...
monophyly.df$Species <- map[monophyly.df$Name, "Species"]
monophyly.df$TreeTool <- map[monophyly.df$Name, "TreeTool"]
monophyly.df$Value <- as.numeric(monophyly.df$Value)

mono.plot.all <- monophyly.df %>% filter(Tool == "StrainPhlan4") %>% ggplot(aes(x=Tool, y=Value, fill=TreeTool, group=TreeTool)) + 
  geom_boxplot() + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
monophyly.df %>% filter(Tool != "gold_std") %>% ggplot(aes(x=reorder(Species, Value, FUN=median, decreasing=TRUE), y=Value, fill=Tool)) + 
  geom_boxplot()

species_order <- monophyly.df %>% filter(Tool == "StrainPhlan4") %>% group_by(Species) %>% summarise(median=median(Value), quantile=quantile(Value)[2], min=quantile(Value)[1]) %>% arrange(median, quantile, min)


monophyly.df %>% filter(Tool != "gold_std") %>% mutate(Species = ordered(Species, levels=rev(species_order$Species))) %>% ggplot(aes(x=Species, y=Value, fill=Tool)) + 
  geom_boxplot()
```


```{r}
#reorder(Species, Value, FUN=median, decreasing=TRUE)
monoplot <- monophyly.df %>% mutate(Species = ordered(Species, levels=rev(species_order$Species))) %>% 
  ggplot(aes(x=Species, y=Value, fill=TreeTool)) + 
  geom_boxplot() + coord_flip()
monoplot


pairwise_plot_iqrax <- monophyly.df %>% 
  filter(Tool == "StrainPhlan4") %>% 
  pivot_wider(id_cols = Group, names_from="TreeTool", values_from="Value") %>%
  left_join(monophyly.df %>% select(Species, Group), by='Group') %>% filter(iqtree != 1 & RaxML != 1) %>%
  ggplot(aes(x=iqtree, y=RaxML, color=Species)) + geom_point(position='jitter') + theme(aspect.ratio=1)
pairwise_plot_iqrax

pairwise_plot_iqvcf <- monophyly.df %>% 
  filter(Tool == "StrainPhlan4") %>% 
  pivot_wider(id_cols = Group, names_from="TreeTool", values_from="Value") %>%
  left_join(monophyly.df %>% select(Species, Group), by='Group') %>% filter(iqtree != 1 & RaxML != 1) %>%
  ggplot(aes(x=iqtree, y=VeryFastTree, color=Species)) + geom_point(position='jitter') + theme(aspect.ratio=1) + xlim(c(0,1)) + ylim(c(0,1))
pairwise_plot_iqvcf
