...
protal_sd <- sd(ds %>% filter(tool == "protal_f1" & metric == "F1" & rank == "Species") %>% pull(value))
protal_mean <- mean(ds %>% filter(tool == "protal_f1" & metric == "F1" & rank == "Species") %>% pull(value))
protal_median <- median(ds %>% filter(tool == "protal_f1" & metric == "F1" & rank == "Species") %>% pull(value))
print(paste("F1:", protal_mean, "+-", protal_sd))

mp4gtdb_sd <- sd(ds %>% filter(tool == "MetaPhlAn4 GTDB" & metric == "F1" & rank == "Species") %>% pull(value))
mp4gtdb_mean <- mean(ds %>% filter(tool == "MetaPhlAn4 GTDB" & metric == "F1" & rank == "Species") %>% pull(value))
print(paste("F1:", mp4gtdb_mean, "+-", mp4gtdb_sd))

mp4_sd <- sd(ds %>% filter(tool == "MetaPhlAn4" & metric == "F1" & rank == "Species") %>% pull(value))
mp4_mean <- mean(ds %>% filter(tool == "MetaPhlAn4" & metric == "F1" & rank == "Species") %>% pull(value))
print(paste("F1:", mp4_mean, "+-", mp4_sd))
...
children <- c()
for (dset in unique(ds.detail$Dataset)) {
  children <- c(children, knitr::knit_child( text=c(paste('### ', dset, sep='')), quiet=TRUE))
  dssub <- ds.detail %>% filter(Dataset == dset & Rank == "Species")
  children <- c(children, knitr::knit_child(input="/usr/users/QIB_fr017/fritsche/Projects/benchpro/knit_abundance_child.Rmd", envir=environment(), quiet = TRUE))
}
cat(children, sep='\n')
```
...
children <- c()
for (r in unique(ds$rank)) {
  children <- c(children, knitr::knit_child( text=c(paste('### ', r, sep='')), quiet=TRUE))
  dssub <- ds %>% filter(rank == r & metric %in% target_stats)
